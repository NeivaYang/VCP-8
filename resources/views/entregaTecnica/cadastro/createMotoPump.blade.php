...
@section('conteudo')
        ...
        <div id="navtabs">
            <ul class="nav nav-tabs" id="myTab" role="tablist">
                ...
                @if ($bombas['id_bomba'] != null && count($bombas['id_bomba'] > 0))
        ...
        <form action="{{ route('save_commissioning_pressurization') }}" method="post" class="mt-3" id="formdados" enctype="multipart/form-data">
            @csrf
                        ...
                        <div class="form-row justify-content-start card_img">
                            ...
                            <div class="form-group col-md-2 mr-5">
                                <div class="card card_pocos" style="width: 250px;">
                                    <div class="card-body">
                                        <input type="radio" name="img_succao" id="pocos" value="pocos" @if(!empty($motobomba['tipo_succao'] == 'pocos')) checked @endif>
                                        <label for="pocos" style="padding: 10px">@lang('entregaTecnica.pocos')</label>
                                        <img src="{{ asset('img/img_succao/pocos.png') }}" id="pocos" class="img-fluid img-succao abrirModal">
                                    </div>
                                </div>
                            </div>
                                                                    ...
                                                                    <div class="d-flex justify-content-start">
                                                                        ...
                                                                        <div>
                                                                            @if (count($motor['plaqueta_img']) > 0)
                                                                                <img src="{{ asset('storage/'. $motor['plaqueta_img'])}}" class="img_preview_editar" onclick="expandImage(this)" id="plaqueta_preview_0{{ $bomba['id_bomba'] }}_0{{$motor['id_motor']}}" data-imgval="1" />   
                                                                            @else                                                             
                                                                ...
                                                                @foreach ($chave_partida as $cp)
                                                                    ...
                                                                    <div class="form-row justify-content-start">
                                                                        ...
                                                                        @php 
                                                                            ...
                                                                            $driveConnectNao = '';
                                                                        @endphp
                                                                        @if ($cp['acionamento'] == "inversora")
                                                                        <div class="form-group col-md-4 input-radio-motobomba" id="drive_connect_0{{ $bomba['id_bomba'] }}_0{{$motor['id_motor']}}_1">
                                                                            @php 
                                                                            if ($cp['drive_connect']) {
                                                                                $driveConnectSim = 'checked';
                                                                            } else {
                                                                                $driveConnectNao = 'checked';
                                                                            }
                                                                            @endphp                                                                          
                                                                        @else
                                                                        ...
                                                                        @endif
                                                                            ...
                                                                            <div class="form-check d-flex pt-2">
                                                                                <div>
                                                                                    <input class="form-check-input" type="radio" name="drive_connect_0{{ $bomba['id_bomba'] }}_0{{$motor['id_motor']}}_1" id="radio-sim" value="1" {{ $driveConnectSim }}>
                                                                                    <label class="form-check-label" for="radio-sim">@lang('comum.sim')</label>                                                                                        
                                                                                </div>
                                                                                
                                                                                <div class="ml-5">
                                                                                    <input class="form-check-input" type="radio" name="drive_connect_0{{ $bomba['id_bomba'] }}_0{{$motor['id_motor']}}_1" id="radio-nao" value="0" {{ $driveConnectNao }}>
        ...
        <!-- Modal imagens Plaqueta do Motor -->
...
@section('scripts')
            ...
            if (quantidade > qtBombaAtual) {
                ...
                for (i = (qtBombaAtual + 1); i <= quantidade; i++) {
                    ...
                    HTML += '                </div>';
                        ...
                        HTML += '                                                       </div>';
                        HTML += '                                                       <div>';
                        //HTML += '                                                           @if ($motor["plaqueta_img"] != null && count($motor["plaqueta_img"]) > 0)';
                        //HTML += '                                                               <img src="{{ asset("storage/". $velocidade["plaqueta_img"])}}" class="img_preview_editar" onclick="expandImage(this)" id="plaqueta_preview__'+ i +'[]" data-imgval="1" />';
                        //HTML += '                                                           @else';
                        ...
                        HTML += '                                                               <img id="plaqueta_preview_'+ i +'" class="img_preview" onclick="expandImage(this)" data-toggle="modal" data-target="#modal_imagem" data-imgval=""/>';
                        //HTML += '                                                           @endif';
                        HTML += '                                                        </div>';
                        HTML += '                                                     </div>';
                        HTML += '                                                 </div>';
    ...
    <script type="text/javascript">
        ...
        //Array de motor_marca em JSON puro
        var motor_marca = [
        ...
        //Array de motor_modelo em JSON puro
        // var motor_modelo = [
        //     for ($i = 0; $i < count($motor_modelo); $i++) {
        ...
        function AdicionarMotor(bomba) {
            ...
            newCard += '                                                       </div>';
            newCard += '                                                       <div>';
            //newCard += '                                                           @if ($motor["plaqueta_img"] != null && count($motor["plaqueta_img"]) > 0)';
            //newCard += '                                                               <img src="{{ asset("storage/". $velocidade["plaqueta_img"])}}" class="img_preview_editar" onclick="expandImage(this)" id="plaqueta_preview_0' + bomba + '_' + qt_card +'" data-imgval="1" />';
            //newCard += '                                                           @else';
