...
{
    // To manage
    public function manageCommissioning() 
    {
    ...
    // To create a new Commissioning
    public function saveCommissioning(Request $request) 
    {
        $dados = $request->all();
        ...
        if ($found_entrega_tecnica == 0) {
            ...
            Session::put('id_created', $id_entrega_tecnica);
            Notificacao::gerarAlert('', 'entregaTecnica.cadastro_entrega_tecnica_sucesso', 'success');
            return redirect()->route('edit_commissioning', $id_entrega_tecnica); 
        } 
        else {
            // if Order Number exists
            Notificacao::gerarAlert('', 'entregaTecnica.erro_pedido_existe', 'warning');
            return redirect()->route('manage_commissioning');
        }
    }

    public function searchCommissioning(Request $request) 
    {
        $entrega_tecnica = [];
    ...
    // Cards
    public function editCommissioning($id_entrega_tecnica)
    { 
        // Check and change, if needed, the Commissioning status.
        ...
        return view('entregaTecnica.editar.editCommissioning', compact('id_entrega_tecnica', 'qtd_chave_partida', 'entrega_tecnica', 'qtd_bombas', 'tem_pivo'));
    }

    public function VerifyDadosCommissioning(Request $request) 
    {
        $dados = $request->all();
    ...
    // Telemetry
    public function createTelemetryCommissioning($id_entrega_tecnica) 
    {
        if (session()->has('fazenda')) {
            $id_fazenda = Session::get('fazenda')['id'];
            $fazenda = Fazenda::select('fazendas.*', 'P.nome as nome_proprietario', 'U.nome as nome_consultor')
    ...
    public function saveTelemetryCommissioning(Request $request) 
    {
        $dados = $request->all();
    ...
    // Commissioning AIR PART
    public function createCommissioningAerialPart($id_entrega_tecnica) 
    {
        if (session()->has('fazenda')) {
            $id_fazenda = Session::get('fazenda')['id'];
            $fazenda = Fazenda::select('fazendas.*', 'P.nome as nome_proprietario', 'U.nome as nome_consultor')
    ...
    public function saveCommissioningAerialPart(Request $req) 
    {
        $dados = $req->all();
        $id_entrega_tecnica = $dados['id_entrega_tecnica'];
    ...
    // Commissioning SPANS
    public function createCommissioningSpan($id_entrega_tecnica) 
    {
        if (session()->has('fazenda')) {
            $id_fazenda = Session::get('fazenda')['id'];
            $fazenda = Fazenda::select('fazendas.*', 'P.nome as nome_proprietario', 'U.nome as nome_consultor')
    ...
    public function saveCommissioningSpan(Request $req)
    {
        $dados = $req->all();
        $id_entrega_tecnica = $dados['id_entrega_tecnica'];

        // checks if you have existing bids in this Commissioning
        $spans = EntregaTecnicaLances::where('id_entrega_tecnica', $id_entrega_tecnica)->count();

        // delete the existing bids in this Commissioning and recreate the bids again
        if ($spans > 0) {
    ...
    // Commissioning MOTORPUMP
    public function createPressurizationCommissioning($id_entrega_tecnica)
    {
        if (session()->has('fazenda')) {
            $id_fazenda = Session::get('fazenda')['id'];
            $fazenda = Fazenda::select('fazendas.*', 'P.nome as nome_proprietario', 'U.nome as nome_consultor')
    ...
    public function savePressurizationCommissioning(Request $request)
    {
        $dados = $request->all();
                ...
                for ($k = 0; $k < count($dados['id_motor_'.$j]); $k++) {
                    ...
                    if ($dados['tipo_motor_' . $id_bomba][$k] == 'eletrico') {
                        ...
                        if ($dados['marca'][$i] != null && $dados['modelo'][$i] != null && $dados['numero_estagio'][$i] != null && 
                            $dados['rotor'][$i] != null && $dados['fornecedor'][$i] != null && 
    ...
    // AUTOTRAFO Commissioning
    public function createStarterKeyCommissioning($id_entrega_tecnica) 
    {
    ...
    public function saveStarterKeyCommissioning(Request $req) 
    {
        $dados = $req->all();
        $id_entrega_tecnica = $dados['id_entrega_tecnica'];
    ...
    // SPRINKLER Commissioning
    public function createSprinklersCommissioning($id_entrega_tecnica)
    {
    ...
    public function saveSprinklersCommissioning(Request $request)
    {
        $dados = $request->all();
        $id_entrega_tecnica = $dados['id_entrega_tecnica'];
    ...
    // SUCTION Commissioning
    public function createSuctionMeasurementsCommissioning($id_entrega_tecnica)
    {
        $succao = EntregaTecnica::select('medicao_succao_l', 'medicao_succao_h', 'medicao_succao_diametro', 'medicao_succao_tipo', 'tipo_tubo_succao')
            ->where('id', $id_entrega_tecnica)->first();
        ...
        return view('entregaTecnica.cadastro.createSuction', compact('id_entrega_tecnica', 'succao_tipo','tipo_succao', 'succao', 'motobomba', 'tubos'));
    }

    public function saveSuctionMeasurementsCommissioning(Request $request)
    {
        ...
        if (!empty($dados['medicao_succao_l']) && !empty($dados['medicao_succao_h']) && !empty($dados['medicao_succao_diametro']) && !empty($dados['tags'])) {
            EntregaTecnica::where('id', $id_entrega_tecnica)->update(['status_succao' => 2]);
        } else if (!empty($dados['medicao_succao_l']) || !empty($dados['medicao_succao_h']) || !empty($dados['medicao_succao_diametro']) || !empty($dados['tags'])) {
    ...
    // Commissioning PRESSURE CONNECTION AND ACCESSORIES
    public function createPressureConnectionMeasurementsCommissioning($id_entrega_tecnica)
    {
    ...
    public function savePressureConnectionMeasurementsCommissioning(Request $request)
    {
        $dados = $request->all();   
        $id_entrega_tecnica = $dados['id_entrega_tecnica'];
        ...
        if (!empty($dados['tubo_az1_diametro']) && !empty($dados['tubo_az2_diametro']) &&
            !empty($dados['registro_gaveta_diametro']) && !empty($dados['peca_aumento_diametro_maior']) &&
            !empty($dados['peca_aumento_diametro_menor']) && !empty($dados['valvula_retencao_diametro'])) {
    ...
    // Commissioning ADDUCTOR
    public function createWaterSupplyMeasurementsCommissioning($id_entrega_tecnica)
    {   
    ...
    public function saveWaterSupplyMeasurementsCommissioning(Request $request)
    {
        $dados = $request->all();
        $id_entrega_tecnica = $dados['id_entrega_tecnica'];
    ...
    public function createTestsCommissioning($id_entrega_tecnica) 
    {
        if (session()->has('fazenda')) {
            $id_fazenda = Session::get('fazenda')['id'];
            $fazenda = Fazenda::select('fazendas.*', 'P.nome as nome_proprietario', 'U.nome as nome_consultor')
        ...
        //////////////////////////////////////////////////////////////////////////////
        // Check if there is record each table related with the Commissioning
            ...
            if ($exist_motor_started == 0) {
                ...
                // Create the first motor stared test this Commissioning
            ...
            if ($exist_central_tower == 0) {
                ...
                // create the first central tower test this Commissioning
            ...
            if ($exist_last_tower_speed == 0) {
                ...
                // Create the first last tower speed test this Commissioning
            ...
            if ($exist_motor_pump == 0) {
                ...
                // Create the first motor pump test this Commissioning
    ...
    public function saveTestsCommissioning(Request $request) 
    {
        $dados = $request->all();
        $id_entrega_tecnica = $dados['id_entrega_tecnica'];
    ...
    public function datasheetCommissioning($id_entrega_tecnica)
    {
        if (session()->has('fazenda')) {
            $fazenda = session()->get('fazenda');            
            $id_fazenda = Session::get('fazenda')['id'];
    ...
    public function sendCommissioning($id_entrega_tecnica)
    {
        if (session()->has('fazenda')) {
            $fazenda = session()->get('fazenda');            
            $id_fazenda = Session::get('fazenda')['id'];
        ...
        return view('entregaTecnica.relatorios.sendCommissioning', compact('id_entrega_tecnica', 'entrega_tecnica'));
    }

    public function sendCompleteCommissioning(Request $request)
    {
        $dados = $request->all();
        $id_entrega_tecnica = $dados['id_entrega_tecnica'];
        if (session()->has('fazenda')) {
        ...
        */
        ...
        if($valida_cpf) {
            ...
            if (isset($dados['cpf'])) {
                ...
                return redirect()->route('send_commissioning', $id_entrega_tecnica);
            } else {
                Notificacao::gerarAlert('', 'entregaTecnica.dados_incompletos', 'warning');
                return $dados;
            }
    ...
    public function manageAnalysisCommissioning() 
    {   
    ...
    public function analysisCommissioning($id_entrega_tecnica) 
    {
    ...
    public function sendAnalisyCommissioning(Request $req)
    {
        $dados = $req->all();
        $id_entrega_tecnica = $dados['id_entrega_tecnica'];
        ...
        return redirect()->route('manage_analysis_commissioning');
    }

    public function sendConfirmationCommissioning(Request $request)
    {
        $credentials = $request->all();

        if(Auth::attempt(['email' => $credentials['email'], 'password' => $credentials['senha']])){
            $dataReturn = ['status' => true, 'message' => 'success'];
        } else {
            $dataReturn = ['status' => false, 'message' => 'error'];
        }
        return response()->json($dataReturn);
    }

    public function checkStatusCommissioning(Request $request) {
        $datas = $request->all();

        $statusCommissioning = EntregaTecnica::select('entrega_tecnica.status')
            ->where('entrega_tecnica.id', $datas['id_entrega_tecnica'])
            ->get();

        return response()->json($statusCommissioning);
    }
    ...
    public function delete($id)
    {
        ...
        $delete->delete();
        return redirect()->route('historic_commissioning')->with('Sucesso', 'Foi deletado');
    }
